...
func (s *Store) Save(ctx context.Context, kind, id string, value any) error {
	...
	_, err = s.db.ExecContext(context.Background(), `INSERT INTO records(kind,id,payload,created_at,updated_at) VALUES(?,?,?,?,?) ON CONFLICT(kind,id) DO UPDATE SET payload=excluded.payload, updated_at=excluded.updated_at`, kind, id, payload, now, now)
	if err != nil {
