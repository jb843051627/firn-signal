...
func (l *Lab) RecordReading(ctx context.Context, input model.RecordReadingInput) (model.ThermalReading, error) {
	...
	scan, err := l.Scan(ctx, input.ScanID)
	if err != nil {
		return model.ThermalReading{}, err
	}
	if !scan.Open() {
	...
	probe, err := l.Probe(ctx, input.ProbeID)
	if err != nil {
		return model.ThermalReading{}, err
	}
	...
	reading := model.ThermalReading{ID: input.ID, ScanID: input.ScanID, ProbeID: input.ProbeID, DepthM: input.DepthM, TempC: input.TempC, Conductivity: input.Conductivity, CollectedAt: input.CollectedAt, Labels: input.Labels}
	if err := l.store.Save(ctx, "reading", reading.ID, reading); err != nil {
		return model.ThermalReading{}, err
	}
	l.metrics.Add("readings.recorded", 1)
	return reading, l.store.Event(ctx, reading.ScanID, "reading-recorded", reading)
}
