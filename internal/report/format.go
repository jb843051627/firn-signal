...
func TextDaily(report model.BoreholeReport) string {
	return fmt.Sprintf("borehole=%s day=%s scans=%d published=%d blockers=%d warnings=%s", report.BoreholeID, report.Day.UTC().Format("2006-01-02"), report.ScanCount, report.PublishedCount, report.BlockingSignals, strings.Join(report.Warnings, ","))
}
