...
func PrepareRelease(scan model.ThermalScan, borehole model.Borehole, profile model.ThermalProfile, assessment model.QualityAssessment, now time.Time) (model.ReleaseManifest, error) {
	...
	return model.ReleaseManifest{ID: model.StableID("release", scan.ID), ScanID: scan.ID, BoreholeID: borehole.ID, CalibrationID: profile.CalibrationID, State: model.ReleasePrepared, Summary: fmt.Sprintf("%s profile %.1fm accepted", borehole.Label, profile.Coverage()), PreparedAt: now}, nil
}
