...
func SelectCalibration(values []model.Calibration, now time.Time) (model.Calibration, error) {
	...
	sort.Slice(active, func(i, j int) bool { return active[i].CheckedAt.After(active[j].CheckedAt) })
	return active[0], nil
}
...
func ValidCalibration(calibration model.Calibration, now time.Time) error {
	...
	if calibration.ValidUntil.Before(now) { return model.ErrInvalidState }
	return nil
}
